...
AacEncoder::~AacEncoder() { aacenc_close(); }

int AacEncoder::aacenc_init(int aot, int channels, int sample_rate,
                            int bitrate) {
  ...
  // -------------------------------------------------------------------------------
  //  ChannelMode           | ChCfg  | front_El      | side_El  | back_El  |
  //  lfe_El
  // -----------------------+--------+---------------+----------+----------+--------
  // MODE_1                 |      1 | SCE           |          |          |
  // MODE_2                 |      2 | CPE           |          |          |
  ...
  if ((err = aacEncoder_SetParam(_h.enc, AACENC_SAMPLERATE, sample_rate)) !=
      AACENC_OK) {
    return err;
  }
  ...
  //      (default)
  //      - 1: WAVE file format channel ordering (e. g. 5.1: L, R, C, LFE, SL,
  //      SR).
  if ((err = aacEncoder_SetParam(_h.enc, AACENC_CHANNELORDER, 1)) !=
      AACENC_OK) {
    return err;
  }

  if ((err = aacEncoder_SetParam(_h.enc, AACENC_BITRATE, bitrate)) !=
      AACENC_OK) {
    return err;
  }

  if ((err = aacEncoder_SetParam(_h.enc, AACENC_TRANSMUX, trans_mux)) !=
      AACENC_OK) {
    return err;
  }

  if ((err = aacEncoder_SetParam(_h.enc, AACENC_SIGNALING_MODE, signaling)) !=
      AACENC_OK) {
    return err;
  }

  if ((err = aacEncoder_SetParam(_h.enc, AACENC_AFTERBURNER, afterburner)) !=
      AACENC_OK) {
    return err;
  }
...
void AacEncoder::aacenc_close() { aacEncClose(&_h.enc); }

int AacEncoder::aacenc_encode(char *pcm, int nb_pcm, int nb_samples, char *aac,
                              int &pnb_aac) {
  AACENC_ERROR err = AACENC_OK;
  ...
  // to make the intput pcm size equals to the encoder calculated
  // size(ibuffer_size). std::cout << ibuffer_size << std::endl;
...
int AacEncoder::aacenc_frame_size() { return _h.frame_size; }
